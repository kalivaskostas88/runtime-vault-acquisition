import cv2, json, sys, pathlib, numpy as np
...
H,W=frames[0].shape[:2]; scale=W/640.0
cuts=[2.52,3.60,6.20]
strums=[0.4,1.0,1.96,3.24,5.36,5.8,6.56,7.68]
blinks=[1.72,4.18,6.62]
place={
 'S1_MASTER':(.205,321,64),
 'S2_MEDIUM':(.285,321,60),
 'S3_CLOSE':(.455,321,-12),
 'S4_RETURN':(.285,320,58)
}
def sid(t):
...
def roi(t,center,size):
    ...
    x0=max(0,int(cx-w/2)); x1=min(W,int(cx+w/2)); y0=max(0,int(cy-h/2)); y1=min(H,int(cy+h/2))
    return x0,y0,x1,y1
...
for i in range(1,len(frames)):
    ...
    for name,center,size in [
      ('mouth',(378,190),(110,80)),
      ('right_hand',(250,505),(150,190)),
      ('left_hand',(497,435),(125,150)),
      ('eyes',(365,133),(100,45)),
      ('face_shell',(365,145),(190,210))
    ]:
...
rep={
 'qa_id':'BARD_EVENT_QA_V1',
 'input':str(p),'frame_count':len(frames),'fps':fps,'resolution':[W,H],
