...
def controls_for_frames(n):
    vals=[]; prev=np.array([28.,0.,0.,0.],np.float32)
    for i in range(n):
...
def mouth_anim(sprite, ctrl):
    ...
    if opn>0.15:
        ...
        sprite[:,:,:3]=np.clip(sprite[:,:,:3].astype(np.float32)*(1-m*.62)+overlay[:,:,:3].astype(np.float32)*(m*.62),0,255).astype(np.uint8)
    return sprite
...
def add_foreground_patrons(bg,scale):
    H,W=bg.shape[:2]; layer=np.zeros((H,W,4),np.uint8)
...
def environment_life(bg, glow, t, phase):
    ...
    return np.clip(bg.astype(np.float32)*(1.0+amp*glow),0,255).astype(np.uint8)
...
def main():
    ...
    ap.add_argument('--tag',default='V1R3F_PROD')
    args=ap.parse_args()
    ...
    for i in range(int(FPS*DUR)):
        t=i/FPS; sid=shot_for(t)
        bg=environment_life(bgs[sid].copy(),glows[sid],t,shot_phase[sid]); p=PLACEMENT_640[sid]
        sf=p['scale']*scale
        sprite=sprite0.copy()
        ...
        vw.write(bg)
    vw.release()
    subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',str(raw),'-i',str(args.audio),
                    '-map','0:v:0','-map','1:a:0','-c:v','libx264','-preset','medium','-crf','18',
                    '-pix_fmt','yuv420p','-r','25','-c:a','aac','-b:a','192k','-shortest',str(final)],check=True)
    probe=json.loads(subprocess.check_output(['ffprobe','-v','error','-show_entries','format=duration,size',
        '-show_entries','stream=codec_type,codec_name,width,height,r_frame_rate,sample_rate,channels','-of','json',str(final)],text=True))
    ...
    rep={'proof_id':f'BARD_GOLDEN_SLICE_{args.tag}','status':'INTEGRATED_RENDER_COMPLETE_PENDING_VIEWER_QA',
         'source_sha256':EXPECTED_SHA,'resolution':[W,H],'fps':FPS,'duration_target':DUR,
         'coverage_cut_times':[2.52,3.60,6.20],'audio_path':str(args.audio),'output':str(final),
         'output_sha256':sha256(final),'ffprobe':probe}
