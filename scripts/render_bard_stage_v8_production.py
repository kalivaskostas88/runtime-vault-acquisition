...
OUT=pathlib.Path("/tmp/bard_stage_v8_production")
OUT.mkdir(parents=True,exist_ok=True)

bpy.ops.wm.open_mainfile(filepath=BLEND)
scene=bpy.context.scene
scene.render.resolution_x=int(os.environ.get("RENDER_W","960"))
scene.render.resolution_y=int(os.environ.get("RENDER_H","540"))
...
cam=scene.camera
if cam is None:
    raise RuntimeError("No active camera in packed V7 master")
proxy=[o for o in bpy.data.objects if o.name.startswith("proxy_")]
if not proxy:
    raise RuntimeError("No proxy objects found in packed V7 master")
...
(OUT/"BARD_STAGE_V8_PRODUCTION_CAMERAS.json").write_text(json.dumps({
  "source_master":"BARD_STAGE_V7_MASTER.blend packed 47 files",
  "resolution":[scene.render.resolution_x,scene.render.resolution_y],
  "requested":requested,
  "shots":selected
},indent=2))
print(f"BARD_STAGE_V8_PRODUCTION_OK requested={requested}")
